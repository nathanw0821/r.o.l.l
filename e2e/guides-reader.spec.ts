...
test.describe("guides reader", () => {
  ...
  test("the contents list marks the section being read", async ({ page }) => {
    ...
    // The visible list shows it. On phones, opening the disclosure scrolls the page up to it, so
    // the mark correctly follows the new reading position: check the open list agrees with it.
    const isPhone = await page.getByText(/^On this page \(\d+\)$/).isVisible();
    const toc = await visibleContents(page);
    if (isPhone) {
      // Opening the disclosure scrolls back up to the intro (no section current there), so read on
      // to Crafting again with the list open.
      await scrollHeadingToTop(page, "crafting");
      await expect(toc.getByRole("link", { name: "Crafting", exact: true })).toHaveAttribute("aria-current", "location");
