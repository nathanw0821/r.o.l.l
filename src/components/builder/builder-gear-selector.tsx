    ...
    <div className="space-y-3 font-mono">
        ...
        {CATEGORY_TABS.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-2xs px-1 rounded ${isSelected ? "bg-slate-950/30 text-slate-950" : "bg-slate-900 text-dim"}`}>
                {count}
              </span>
            </button>
          );
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
        {activeCategory === "weapon" ? (
          <div className="flex flex-wrap gap-1">
            {WEAPON_SUB_TABS.map((subTab) => (
              <button
                ...
                className={`px-2 py-1 rounded text-2xs font-bold uppercase border transition-all ${
          ...
          <div className="text-2xs text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div className="relative w-full sm:w-56">
          ...
          <input
            ...
            className="w-full pl-8 pr-7 py-1 rounded border border-slate-800 bg-slate-900/90 text-xs text-slate-200 placeholder:text-dim focus:outline-none focus:border-emerald-500 font-mono"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery("")}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-dim hover:text-slate-300"
            >
              <X className="h-3 w-3" />
            </button>
          )}
      ...
      {activeCategory === "underarmor" && underarmor && onUnderarmorChange && (
        <div className="p-3.5 rounded-xl bg-slate-950/90 border border-emerald-500/40 shadow-md space-y-3 font-mono">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2 text-xs font-black uppercase tracking-wider text-emerald-400">
            ...
            {inPowerArmor ? (
              <span className="text-2xs px-2 py-0.5 rounded bg-amber-950/80 text-amber-400 border border-amber-500/40 font-bold">
                ⚠️ SUPPRESSED IN POWER ARMOR
              </span>
            ) : (
              <span className="text-2xs px-2 py-0.5 rounded bg-emerald-950/80 text-emerald-300 border border-emerald-500/40 font-bold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            ...
            <div className="space-y-1.5 rounded-lg bg-slate-900/80 border border-slate-800 p-2.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-3xs text-cyan-400 font-bold">
            ...
            <div className="space-y-1.5 rounded-lg bg-slate-900/80 border border-slate-800 p-2.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-3xs text-amber-400 font-bold">
              ...
              >
                <div className="flex items-start justify-between gap-2 min-w-0 w-full">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-3xs text-slate-400 uppercase tracking-wider font-mono truncate">
