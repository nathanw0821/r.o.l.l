...
describe("guide list URL state", () => {
  ...
  it("reads every param, including the ?query= alias", () => {
    expect(
      parse("query=fixer&category=Patch%20notes%20%26%20news&update=The-Pitt&archive=1&source=NukaKnights&stubs=hide&page=3"),
    ).toEqual({
      q: "fixer",
  ...
  it("ignores unknown or invalid values", () => {
    expect(parse("category=Nope&update=nope&source=Reddit&stubs=show&archive=yes&page=-2")).toEqual(DEFAULT_GUIDE_LIST_STATE);
  ...
  it("round-trips through the query string and omits defaults", () => {
    ...
    expect(qs).toBe("q=bloodied+build&category=Weapons+%26+Mods&update=skyline-valley&archive=1&source=Fallout+Wiki&stubs=hide&page=2");
  ...
  it("builds removable chips and clears one filter back to page 1", () => {
    const state = parse("q=fixer&category=Weapons%20%26%20Mods&source=NukaKnights&update=the-pitt&archive=1&stubs=hide&page=4");
    expect(countActiveFilters(state)).toBe(5);
    const chips = activeFilterChips(state, (id) => (id === "Weapons & Mods" ? "Weapons & legendary mods" : id));
    expect(chips.map((c) => c.key)).toEqual(["q", "category", "source", "update", "archive", "stubs"]);
